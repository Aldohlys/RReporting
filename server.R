...
server <- function(input, output, session) {
  ...
  observeEvent(input$loadTrades, {
                ...
                updateSliderInput(inputId = "init_date",min=date_min,max=today(),value=today()-20)
  })
  ...
  sub_all_trades = reactive({
    ...
    if (!is.null(data)) {
      ...
      data %<>% filter(TradeDate >= ymd(input$init_date),
                        TradeNr >=input$init_nr)
      data
    }
  })
  ...
  ### TradeNr;Account;TradeDate;Theme;Description;Ssjacent;Pos;Prix;Comm.;Total;Exp.Date;
  ###    Risk;Reward;PnL;Statut;Curr.;Remarques
  ...
  insert_new_trade = function(account,trade_nr, theme,risk, reward, text,data) {
    ...
    ### TradeDate;Description;Ssjacent;Pos;Prix;Comm.;Total;Exp.Date;Statut;Curr.
    new_trade = data
